...
def evaluate(args, net, dataset, segment="valid"):
    ...
    with th.no_grad():
        ...
        # print("pred_ratings:", th.softmax(pred_ratings, dim=1))
    real_pred_ratings = (
        th.softmax(pred_ratings, dim=1) * nd_possible_rating_values.view(1, -1)
    ).sum(dim=1)
    #pre_lables = pred_ratings.max(1)[1]
    # print("rating_values:", rating_values)
    rmse = ((real_pred_ratings - rating_values) ** 2.0).mean().item()
    #pred_labels = _make_labels(real_pred_ratings)
    #print("real_pred_ratings:", real_pred_ratings)
    rmse = np.sqrt(rmse)
    pred_label = real_pred_ratings.cpu().numpy()
    truth_label = rating_values.cpu().numpy()

    return rmse, pred_label, truth_label
...
def train(args):
    ...
    for iter_idx in range(1, args.train_max_iter):
        ...
        if iter_idx % args.train_valid_interval == 0:
            valid_rmse, valid_label, truth_label= evaluate(
            ...
            if valid_rmse < best_valid_rmse:
                ...
                no_better_valid = 0
                best_iter = iter_idx
                test_rmse, pred_label, truth_label = evaluate(
    ...
    np.savetxt(r'truth_label.txt', best_truth_label, fmt='%.3f', delimiter=',')
    np.savetxt(r'test_FG.txt', best_pred_label, fmt='%.3f', delimiter=',')
...
def config():
    ...
    parser.add_argument("--gcn_agg_units", type=int, default=800)
