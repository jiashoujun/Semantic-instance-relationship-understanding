...
if __name__=='__main__':
    ...
    import time
    np.random.seed(100)
    pts = np.random.random((12,8,9)).astype('float32')
    tmp1 = np.random.random((12,8,3)).astype('float32')
    tmp2 = np.random.random((12,4,3)).astype('float32')
    with tf.device('/gpu:1'):
        ...
        if knn:
            ...
            neighbor_idx = nearest_neighbors.knn_batch( tmp1, tmp2,nsample,omp=True)
